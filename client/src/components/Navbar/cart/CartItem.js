...
const CartItem = ({ item }) => {
  ...
  const changeQuantity = q => {
    if (q === 1) {
      dispatch(
        changeProductQuantity({ id: item._id, quantity: item.quantity + 1 })
      );
  ...
  return (
    <div className='cartItem row justify-content-between'>
      ...
      <div className='col-12 col-md-3 midCol'>
        <form className='productSize secondary-font'>
          <div
            className='btn-group'
            role='group'
        ...
        <p className='productQuantity secondary-font'>
          <svg
      ...
      <div className='col-12 col-md-3 endCol'>
        <span></span>
        <p className='productPrice secondary-font'>
          ${String((item.price.unit_amount / 100) * item.quantity)}
        </p>
        <button
