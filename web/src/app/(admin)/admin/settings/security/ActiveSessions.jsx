...
const ActiveSessions = () => {
  ...
  const sessions = useSelector(
    state => state.sessions || { data: [], loading: false, error: null }
  );
  ...
  const handleEndSession = async sessionId => {
    const result = await dispatch(endSessionAction(sessionId));
    if (result.success) {
      showSuccess('Session ended successfully');
      dispatch(fetchSessions()); // Reload sessions
    } else {
  ...
  const handleEndAllOtherSessions = async () => {
    const result = await dispatch(endAllOtherSessionsAction());
    if (result.success) {
